...
export default function EmployeeCRM() {
  ...
  const [employeeSipSettings, setEmployeeSipSettings] =
    useState<EmployeeSipSettings | null>(null);
  ...
  const [statusFilter, setStatusFilter] = useState<CRMLeadStatus | "all">(
    "all"
  );
  ...
  useEffect(() => {
    const loadEmployeeData = async () => {
      ...
      if (sessionEmployee) {
        ...
        // Load company data
        const companyData = await getCompanyById(sessionEmployee.company_id);
        if (companyData) {
      ...
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-2">
          ...
          <select
            value={statusFilter}
            onChange={(e) =>
              setStatusFilter(e.target.value as CRMLeadStatus | "all")
            }
              ...
              <CardContent className="p-6">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      >
                        {lead.status === "Yeni"
                          ? t("crm.status.new")
                          : lead.status === "Görüşüldü"
                          ? t("crm.status.contacted")
                          : lead.status === "Satış Yapıldı"
                          ? t("crm.status.sold")
                          : lead.status === "Reddedildi"
                          ? t("crm.status.rejected")
                    ...
                    <div className="flex flex-wrap gap-4 text-sm text-muted-foreground">
                      {lead.phone && (
                        <span>
                          {t("crm.phone")}: {lead.phone}
                        </span>
                      )}
                      {lead.email && (
                        <span>
                          {t("crm.email")}: {lead.email}
                        </span>
                      )}
                      {lead.follow_up_date && (
                        <span>
                  ...
                  <div className="flex items-center gap-2 ml-4">
                    {lead.phone && employeeSipSettings && (
                      <CallButton
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        handleStatusChange(
                          lead.id,
                          e.target.value as CRMLeadStatus
                        )
                      }
                      className="px-2 py-1 text-xs border border-gray-300 rounded bg-white"
                    >
                      <option value="Yeni">{t("crm.status.new")}</option>
                      <option value="Görüşüldü">
                        {t("crm.status.contacted")}
                      </option>
                      <option value="Satış Yapıldı">
                        {t("crm.status.sold")}
                      </option>
                      <option value="Reddedildi">
                        {t("crm.status.rejected")}
                      </option>
                      <option value="Takipte">
                        {t("crm.status.followUp")}
                      </option>
                    </select>
                    <Button
          ...
          {(
            ...
            return (
              <div key={status} className="flex flex-col min-w-[250px]">
                <div
                  className={`p-3 rounded-t-lg border-b-2 ${statusColors[status]}`}
                >
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    status: e.target.value as CRMLeadStatus,
                  })
                }
