...
import type { CRMLead, CRMLeadFormData, CRMLeadStatus, CRMStats, Employee } from '@/types'
...
export default function CRM() {
  ...
  const loadStats = async () => {
    ...
    const statsData = await getCRMStats(companyId)
    setStats(statsData)
  }
  ...
  const handleSave = async () => {
    if (!companyId || !formData.customer_name) return
    ...
    if (editingLead) {
      ...
      const created = await createLead(companyId, formData)
      if (created) {
        await loadLeads()
        await loadStats()
        handleCloseDialog()
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent onClose={handleCloseDialog}>
          ...
          <div className="space-y-4">
            ...
            <div>
              <Label htmlFor="employee_id">{t('crm.assignedEmployee') || 'İlgilenen Çalışan'}</Label>
              <select
                id="employee_id"
                ...
                className="w-full px-3 py-2 border border-gray-300 rounded-lg"
              >
                <option value="">{t('crm.noEmployee') || 'Çalışan Seçilmedi'}</option>
