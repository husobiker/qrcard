...
export async function updateCallLog(
  ...
  try {
    ...
    if (callData.customer_name !== undefined)
      updateData.customer_name = callData.customer_name || null;
    if (callData.customer_id !== undefined)
      updateData.customer_id = callData.customer_id || null;
    ...
    if (callData.recording_url !== undefined)
      updateData.recording_url = callData.recording_url || null;
    if (callData.notes !== undefined) updateData.notes = callData.notes || null;
    if (callData.call_start_time !== undefined)
      updateData.call_start_time = callData.call_start_time;
    if (callData.call_end_time !== undefined)
      updateData.call_end_time = callData.call_end_time || null;
...
export async function getCallLogStats(
  ...
  try {
    ...
    const totalDuration = logs.reduce(
      (sum: number, log: any) => sum + (log.call_duration || 0),
      0,
    );
