...
export default function EmployeeReportsScreen({ navigation }: any) {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const statsPromises = employeesData.map(async (employee) => {
        const [taskStats, quotes, leads] = await Promise.all([
          getTaskStats(companyId, employee.id),
          getQuotes(companyId, employee.id),
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]} edges={["left", "right", "top"]}>
      ...
      >
        <Text style={[styles.headerTitle, { color: theme.colors.text }]}>Personel Raporları</Text>
      </View>
